...
# GitHub       : https://github.com/slayerrider0097-byte/Arush Music
#
...
def run_http_server():
    ...
    logger.info(f"馃寪 HTTP health check server started on port {port}")
...
async def main():
    try:
        ...
        http_thread.start()
        logger.info("馃寪 HTTP server thread started for Render health checks")
        ...
        logger.info(f"馃攲 Loaded {len(all_modules)} plugin modules.")
        ...
        logger.info(f"馃憫 Loaded {len(app.sudoers)} sudo users.")
        logger.info("\n馃帀 Bot started successfully! Ready to play music! 馃幍\n")
