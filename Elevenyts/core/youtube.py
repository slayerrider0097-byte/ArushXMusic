...
class YouTube:
    def __init__(self):
        ...
        self.arush_key = config.ARTISTBOTS_KEY
        ...
        if self.enable_api:
            ...
            if self.arush_key:
                masked_key = self.arush_key[:8] + "..." if len(self.arush_key) > 8 else "***"
    ...
    async def download_via_api(self, link: str, video: bool = False) -> Optional[str]:
        """
        Download audio/video using Arush API (Primary Method).
        ...
        try:
            ...
            logger.info(f"🚀 [API PRIMARY] Trying Arush API for {video_id} (type: {download_type})")
            ...
            if self.arush_key:
                params["api_key"] = self.arush_key
                logger.debug(f"Using API key: {self.arush_key[:8]}...")
            else:
                logger.warning("No Arush API key configured!")
                return None
    ...
    async def download(self, video_id: str, is_live: bool = False, video: bool = False) -> Optional[str]:
        ...
        if self.enable_api and self.api_url and self.arush_key:
